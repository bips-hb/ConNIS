...
Using a simple "Bonferroni correction" with $\alpha=0.05$ for multiple testing problem ConNIS declared the follwing 21 genes as essential:
...
```
16 genes are declared essential since smaller weights will make it harder to label a gene (by chance) as 'essential'. 

Next, we give an example how to select a weight by the instability approach. We will use the parallel version of the function using `mclapply`. We use different weights. NOTE: We use only 100 subsamples for demonstration purpose. For real world applications we suggest $m \approx  500$ and parallel processing uization of the .
